        ...
        ZStack {
            ...
            ScrollView{
                VStack {
                    ...
                    Text("Email findmybuses@gmail.com")
                        .font(.body)
                        .bold()
                        .padding(.bottom, 20)
...
struct ContentView: View {
    ...
    @State private var emailNotVerified = false // State for showing resend button
                        ...
                        ScrollView {
                            ...
                            VStack(spacing: 15) {
                                ...
                                if let errorMessage = errorMessage {
                                    Text(errorMessage)
                                        .foregroundColor(errorMessage == "Verification email sent successfully." ? .green : .red) // Change to green for success, red for other messages
                                        .font(.subheadline)
                                        .padding(.horizontal, 20)
                                }

                                // Show Resend Verification Email button when email is not verified
                                if emailNotVerified {
                                    Button(action: {
    ...
    private func login() {
        Auth.auth().signIn(withEmail: email, password: password) { result, error in
            ...
            if user.isEmailVerified {
                ...
                emailNotVerified = true // Set this to true to show the "Resend Verification Email" button
                
            }
        }
    }
    
    private func fetchUserDetails(_ user: User) {
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                Color(red: 0.57, green: 0.77, blue: 1.5)
        ...
        if let user = user {
            ...
            db.collection("users").document(user.uid).getDocument { document, error in
                ...
                if let data = document?.data() {
                    ...
                    case "Flanagan High School":
                        logoImageName = "flanagan_logo"
                    default:
                        logoImageName = ""
                    }
                }
...
struct CalendarView: View {
    ...
    var body: some View {
        ...
        .background(Color(red: 0.9, green: 0.95, blue: 1.5))
        .environment(\.colorScheme, .light)
...
struct BusTableView: View {
    ...
    var body: some View {
        NavigationView {
            ZStack {
                Color(.init(red: 185.0 / 255.0, green: 185.0 / 255.0, blue: 185.0 / 255.0))
                ...
                ScrollView {
                    VStack {
                        ...
                        if schoolName == "Pompano Beach High School" {
                                                Image("pb_loop")
                                                    .resizable()
                                                    .aspectRatio(contentMode: .fit)
                                                    ...
                                                    .padding(.top, 5)
                                                    .padding(.bottom, -30)
                                            } else if schoolName == "Flanagan High School" {
                                                Image("flanagan_loop")
                                                    .resizable()
                                                    .aspectRatio(contentMode: .fit)
                                                    .frame(width: 300, height: 170)
                        ...
                        Spacer()
                        if showAlert {
                                           Text("You can only save information for today's date.")
                                               .foregroundColor(.red)
                                               .padding(.top,-30)
                                               .padding(.bottom,-50)
                                               .transition(.opacity)
...
struct MoreBusInfoView: View {
    let schoolName: String
    @State private var busInfo: String = ""
    @State private var isAdmin: Bool = false
    @State private var successMessage: String? = nil
    @State private var userEmail: String? = nil
    @State private var messages: [BusMessage] = []
    @State private var showAlert = false
    @State private var messageToDelete: BusMessage? = nil

    @StateObject private var userAuth = UserAuth()

    // DateFormatter to format the timestamp
    ...
    func deleteMessage() {
        ...
        guard let currentUserEmail = userAuth.currentUser?.email else {
            print("Current user email is nil.")
            return
        }

        if message.updatedBy != currentUserEmail {
        ...
        db.collection("bus_messages").document(schoolName).collection("messages").document(messageId).delete { error in
            if let error = error {
         ...
         ZStack {
             ...
             VStack {
                 ...
                 ScrollView {
                         ...
                         ForEach(messages) { message in
                             VStack(alignment: .leading) {
                                 ...
                                 Text("Posted by: \(message.updatedBy)")
                                     ...
                                     .background(Color.clear)
                                     .environment(\.colorScheme, .light)
                                 Text("Timestamp: \(timeFormatter.string(from: message.timestamp.dateValue()))")
                                     .font(.subheadline)
                                     .foregroundColor(.gray)
                                     .background(Color.clear)
                                     .environment(\.colorScheme, .light)
                                 ...
                                 HStack {
                                     Spacer()
                                     if message.updatedBy == userAuth.currentUser?.email {
                                         Button(action: {
                             ...
                             .padding()
                             .background(Color.white) // Set background color of each message item
                             .cornerRadius(10)
                             .shadow(radius: 5)
                             .padding(.bottom, 5)
                             .onTapGesture {
                     ...
                     .padding()
                     .background(Color.gray.opacity(0.1)) // Set background color of the scrollable area
                     .cornerRadius(10)
                 }
                 .padding()
    ...
    private func fetchUserRole() {
        guard let userEmail = userAuth.currentUser?.email else { return }
    ...
    private func saveBusInfo() {
        guard let userEmail = userAuth.currentUser?.email else {
        ...
        db.collection("bus_messages").document(schoolName).collection("messages").addDocument(data: [
            ...
            if let error = error {
                ...
                successMessage = "Failed to save. Please try again."
            } else {
...
struct SignUpView: View {
    ...
    @State private var preferredBusNumber: String = "" 
    
    private let roles = ["Please select your role", "Student", "Admin"]
    @State private var role: String = "Please select your role"
    
    // Dictionary for school admin passwords
    private let schoolAdminPasswords: [String: String] = [
        "Flanagan High School": "admin12800FHS!",
        "Pompano Beach High School": "admin33060PBHS$"
    ]
    
    private let schools = ["Flanagan High School", "Pompano Beach High School"].sorted() // List of schools
                ...
                VStack(spacing: 12) { // Increased spacing between fields
                    ...
                    SecureField("Confirm Password", text: $confirmPassword)
                        ...
                        Picker("Role", selection: $role) {
                            //Text("Please select your role").tag("")
                    ...
                    .onChange(of: role) { newValue in
                        isAdminPasswordRequired = newValue == "Admin"
                                       ...
                                       Picker("School", selection: $selectedSchool) {
                                           Text("Please select a school").tag("")
                    ...
                    if !isSignUpSuccessful, let errorMessage = errorMessage {
                        Text(errorMessage)
                            .foregroundColor(.red)
                    ...
                    .padding(.top, 20)
                    .padding(.bottom, 30)// Increased top padding
